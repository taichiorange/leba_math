...
def bit_reservation(pai, length):
    """
    对 pai 数组进行比特反转重排。
    pai: 一个列表，长度为 length
    """
    ...
    for index in range(length):
        ...
        for j in range(1, n+1):
            bit_string[j-1] = temp % 2
            temp //= 2
        pai[index] = sum(bit_string[j-1] * (2 ** (n - j)) for j in range(1, n+1))

def transform(cc, length):
    """
    对 cc 列表进行变换操作（分组内按位异或）。
    """
    i = 1
    while i < length:
