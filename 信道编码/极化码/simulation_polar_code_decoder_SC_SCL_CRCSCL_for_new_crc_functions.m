...
snr=2;   %高斯信道信噪比
L=16;  %CASCL译码器的列表长度
crc_obj = get_crc_objective(crc_len);%输出对应的CRC生成器与CRC校验器
...
info_with_crc = crcGenerate(info, crc_obj); 
...
for i_run = 1 : max_runs
        ...
        polar_info_cascl = CASCL_decoder(llr, L, K, frozen_bits, crc_obj, lambda_offset, llr_layer_vec, bit_layer_vec);
...
for phi = 0 : N - 1
    switch phi
        ...
        otherwise
            llr_layer = llr_layer_vec(phi + 1); % 0
            index_1 = lambda_offset(llr_layer + 1); % 1
            index_2 = lambda_offset(llr_layer + 2); % 2
            % 这个是计算前面已经有译码出来 u 的N=2极化码的 似然比
            for beta = index_1 : index_2 - 1%g function is first implemented.
                % P(1) = (1 - 2 * C(1, 1)) * P(1 + 1) + P(1 + 2)
            ...
            for i_layer = llr_layer - 1 : -1 : 0%then f function is implemented.
                ...
                index_2 = lambda_offset(i_layer + 2);
                for beta = index_1 : index_2 - 1
                    %P(1) = sign(P(1 + 1)) * sign(P(1 + 2)) * min(abs(P(1 + 1)), abs(P(1 + 2)));
                    P(beta) =  sign(P(beta + index_1)) * sign(P(beta + index_2)) * min(abs(P(beta + index_1)), abs(P(beta + index_2))); % P(1)
                end
            end
    end
    phi_mod_2 = mod(phi, 2);
...
for i = 1:log2(N)
    ...
    for k = 1:j
        tmp = u(k);
        u(k) = phi_inverse(1 - (1 - phi_func(tmp))^2);
        u(k + j) = 2 * tmp;
    end
end
u = bitrevorder(u);
...
function crc_obj = get_crc_objective(crc_length)
    switch crc_length
        case 4
            crc_obj = crcConfig(Polynomial = "x^4 + x + 1");
        case 6
            crc_obj = crcConfig(Polynomial = "x^6 + x + 1");
        case 8
            crc_obj = crcConfig(Polynomial = "x^8 + x^2 + x + 1");
        case 10
            crc_obj = crcConfig(Polynomial = "x^10 + x^9 + x^5 + x^4 + x + 1");
        case 12
            crc_obj = crcConfig(Polynomial = "x^12 + x^11 + x^3 + x^2 + 1");
        case 16
            crc_obj = crcConfig(Polynomial = "x^12 + x^11 + x^3 + x^2 + 1");
        case 24
            crc_obj = crcConfig(Polynomial = "x^24 + x^23 + x^6 + x^5 + x + 1");
        otherwise
...
if (y <= 1.0221) && (y >= 0.0388)
    ...
    x0 = 0.0388;
    x1 = x0 - (phi_func(x0) - y)/derivative_phi(x0);
    delta = abs(x1 - x0);
    epsilon = 1e-3;

    while(delta >= epsilon)
        x0 = x1;
        x1 = x1 - (phi_func(x1) - y)/derivative_phi(x1);
        %当x1过大，放宽epsilon
        if x1 > 1e2
            epsilon = 10;
        end
...
if (x >= 0)&&(x <= 10)
    dx = -0.4527*0.86*x^(-0.14)*phi_func(x);
else
...
function polar_info_esti = CASCL_decoder(llr, L, K, frozen_bits, ...
   crc_obj,lambda_offset, llr_layer_vec, bit_layer_vec)
%LLR-based SCL deocoder, a single function, no other sub-functions.
%Frequently calling sub-functions will derease the efficiency of MATLAB
%codes.
%const
...
%     err = sum(mod(H_crc * info_with_crc, 2));
    [~,err]=crcDetect(info_with_crc, crc_obj);
    if err == 0
